...
data "aws_iam_policy_document" "apprunner_assume" {
  statement {
    ...
    principals { type = "Service", identifiers = ["build.apprunner.amazonaws.com", "tasks.apprunner.amazonaws.com"] }
  }
}
...
data "aws_iam_policy_document" "apprunner_policy" {
  ...
  statement {
    ...
    resources = [
      ...
      aws_secretsmanager_secret.db_app_creds.arn   # << NOVO
    ]
  }
  statement {
    actions   = ["logs:*"]
...
# 1) API Intermediária (Java/Spring) — igual, só lendo secrets acima
resource "aws_apprunner_service" "api_intermediaria" {
  ...
  health_check_configuration { path = "/actuator/health", interval = 10, timeout = 5, healthy_threshold = 1, unhealthy_threshold = 5 }
  tags = local.labels
}

# 2) API do Agente (Python/FastAPI) — igual, mas DATABASE_URL vem do local
resource "aws_apprunner_service" "api_agente" {
  ...
  health_check_configuration { path = "/health", interval = 10, timeout = 5, healthy_threshold = 1, unhealthy_threshold = 5 }
  tags = local.labels
}

# 3) API Principal (Java/Spring)
# >> ALTERADO: AGENTE_URL agora referencia diretamente o service_url do App Runner do agente
